...
class GraphSAM(torch.optim.Optimizer):
    ...
    def first_step(self, zero_grad=False, i=0):
        ...
        # g_h2 = alpha * g_h1 + (1-alpha) * g_s1
            ...
            for k in range(len(gh_par)):
                if self.gh_grad[k] is not None:
                    self.gh_grad[k] = (1 - self.alpha) * (
                        self.gs_grad[k] / (torch.norm(self.gs_grad[k] + 1e-8))
                    ) + self.alpha * self.gh_grad[k]
        ...
        for j in range(len(gh_par)):
            ...
            e_w = (
                (torch.pow(p, 2) if self.adaptive else 1.0) * grad * scale.to(p)
            )
            # e_w.clamp_(-self.radius, self.radius)
            norm = torch.norm(e_w)
            if norm > self.radius:
...
class LookSAM(torch.optim.Optimizer):
    def __init__(
        self, k, alpha, model, base_optimizer, criterion, rho=0.05, **kwargs
    ):
        """
    ...
    def step(self, t, closure=None, zero_grad=False):
        ...
        for group in self.param_groups:
            scale = (
                group["rho"] / (self._grad_norm() + 1e-12)
                if not t % self.k
                else None
            )
            ...
            for index_p, p in enumerate(group["params"]):
                if not t % self.k:
                    ...
                    self.state[f"gv_{index_p}"]["gv"] = torch.sub(
                        p.grad,
                        p.grad.norm(p=2)
                        * torch.sum(g_grad_norm * g_s_grad_norm)
                        * g_grad_norm,
                    )
