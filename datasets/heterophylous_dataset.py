...
class Dataset:
    def __init__(
        ...
        if do_not_use_original_features and not any(
            [use_sgc_features, use_identity_features, use_adjacency_features]
        ):
            raise ValueError(
                "If original node features are not used, at least one of the arguments "
        ...
        train_idx_list = [
            torch.where(train_mask)[0] for train_mask in train_masks
        ]
        ...
        self.train_idx_list = [
            train_idx.to(device) for train_idx in train_idx_list
        ]
    ...
    def compute_metrics(self, logits):
        if self.num_targets == 1:
            train_metric = roc_auc_score(
                y_true=self.labels[self.train_idx].cpu().numpy(),
                y_score=logits[self.train_idx].cpu().numpy(),
            ).item()

            val_metric = roc_auc_score(
                y_true=self.labels[self.val_idx].cpu().numpy(),
                y_score=logits[self.val_idx].cpu().numpy(),
            ).item()

            test_metric = roc_auc_score(
                y_true=self.labels[self.test_idx].cpu().numpy(),
            ...
            train_metric = (
                (preds[self.train_idx] == self.labels[self.train_idx])
                .float()
                .mean()
                .item()
            )
            val_metric = (
                (preds[self.val_idx] == self.labels[self.val_idx])
                .float()
                .mean()
                .item()
            )
            test_metric = (
                (preds[self.test_idx] == self.labels[self.test_idx])
                .float()
                .mean()
                .item()
            )
    ...
    def augment_node_features(
        ...
        node_features,
        use_sgc_features,
        use_identity_features,
        use_adjacency_features,
        ...
        if use_sgc_features:
            sgc_features = Dataset.compute_sgc_features(
                graph, original_node_features
            )
